function [posterior,out] = VBA_fminunc_wrapper(y,u,f_fname,g_fname,dim,options)
...
tic;
% % manual dev
% fname = @e_EXPLORE_IOWA;
% gname = @o_EXPLORE_IOWA;
% y = A.fit.y{1};
% u = A.fit.u{1};
...
if ~isfield(options, 'isYout')
    ...
    isYout = zeros(size(y));
    options.isYout = options.isYout + isYout;
end

%% options of the fminunc algorithm
try
    fit_options = optimoptions('fminunc', 'algorithm', 'quasi-newton', 'display', 'notify-detailed', 'MaxFunEvals',500, 'TolFun', options.TolFun);
catch
    fit_options = optimset('algorithm', 'lm-line-search', 'display', 'notify-detailed', 'MaxFunEvals',500, 'TolFun', options.TolFun);
end
%% iterate fminunc over starting points
st = 1;
st_all = 1;
options.ind_param = [ones(1,dim.n_theta) 2*ones(1,dim.n_phi)];
while st<=options.fminunc_n_init_random
    % generate starting parameters
    for pt = 1:dim.n_theta
    init_param(st,pt) = options.priors.muTheta(pt)+options.priors.SigmaTheta(pt,pt)*randn;
    end
    for pp = 1:dim.n_phi  
    init_param(st,pt+pp) = options.priors.muPhi(pp)+options.priors.SigmaPhi(pp,pp)*randn;
    end
%     error()
    % try optimization (may fail if undefined at starting point (e.g
    % inverse temperature of softmax to high)
    try
       [opt_param(st,:),opt_NLL(st,1),opt_exitflag(st),opt_output{st}] = fminunc(@(param) integrated_function(param, y, u, options, f_fname, g_fname), init_param(st,:), fit_options);
       disp(['Starting point ' num2str(st) ' on ' num2str(options.fminunc_n_init_random)])
       st = st+1;
    catch 
       disp(['Starting point ' num2str(st) ' failed: Redo'])        
    end
    st_all = st_all + 1;
end

%% keep only the best fitting parameters
min_NLLind = find(opt_NLL==min(opt_NLL),1,'first');
[final_NLL, final_fx, final_gx, final_param] = integrated_function(opt_param(min_NLLind,:), y, u, options,f_fname, g_fname);

%% fill posterior structure
posterior.muPhi = final_param(options.ind_param==2);
posterior.SigmaPhi = [];
posterior.muTheta = final_param(options.ind_param==1);
...
posterior.b_alpha = [];
posterior.iQx = [];
posterior.muX = final_fx;
posterior.SigmaX = [];
posterior.gx = final_gx; % predicted observations (usually in out.suffStat)
...
out.dim = dim;
out.suffStat = [];
out.suffStat.gx = final_gx;
out.date = clock;
out.toolbox = VBA_version();
out.dt = toc;

% build out.fit structure
out.fit.ny = sum(sum(options.isYout)==0,2);
out.fit.ntot = dim.n_theta+dim.n_phi;
out.fit.LL = -final_NLL;
out.fit.AIC = out.fit.LL -(dim.n_theta+dim.n_phi);
out.fit.BIC = out.fit.LL -0.5*out.fit.ntot*log(out.fit.ny);
out.fit.exitflag = opt_exitflag(min_NLLind);
out.fminunc_output = opt_output{min_NLLind};
out.start_attempts = st_all;
out.starts = options.fminunc_n_init_random;


end

% negative loglikelihood of the model given the parameters
function [NLL, fx, gx, param] = integrated_function(param, y, u, options, f_fname,g_fname)
x(:,1) = options.priors.muX0;
param_f = param(options.ind_param==1);
param_g = param(options.ind_param==2);
NLL = 0;
for t = 1:size(y,2)
    fx(:,t) = f_fname(x(:,t),param_f,u(:,t),options.inF);
    gx(:,t) = g_fname(fx(:,t),param_g,u,options.inG);
    if sum(options.isYout(:,t))>0
        % skip NLL computation
    else
        NLL=NLL-log(gx(logical(y(:,t)'),t));
    end
    x(:,t+1) = fx(:,t);
end
fx = x(:,1:end-1);
...
% helper function for fminunc
function NLL = helper_func(param, y, u, options, f_fname,g_fname)

NLL = integrated_function(param, y, u, options, f_fname,g_fname);
