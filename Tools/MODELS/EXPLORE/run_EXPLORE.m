...
switch A.fit.priors.type
    ...
    case 'secondpass'
        ...
        % get results from the first-pass and compute relative priors
        % the path should be written manually relative to IGT_main script.
        options.priors.first_pass = [A.main_path '/504s_vba_flat/o_EXPLORE_IOWA_e_EXPLORE_IOWA_13-Jun-2017_1/fitted_model.mat'];
        ...
        for s = 1:size(R.rawMuTheta,1)
            ...
            priors{s}.muX0(1:8,1) = [0 0 0 0 1 1 1 1];%hs.initval;
        end
        Traw = @(x) x;
        Tsig = @sig;
        Texp = @exp;
    ...
    case 'informed2'
        ...
        % Priors' distributions derived from the second pass
        
        priors.muPhi = [-0.903915969022757,0.0547433302291274];
        priors.SigmaPhi = [0.282503396617074,0;0,1.54612171669429];
        priors.muTheta = [-1.71303986081042,0.590615838194465,-0.795085924140179,0.0280697702655463];
        priors.SigmaTheta = [0.913095911279356,0,0,0;0,1.12549213813182,0,0;0,0,0.689369654221441,0;0,0,0,0.713544034716666];
        priors.muX0(1:8,1) = [0 0 0 0 1 1 1 1];%hs.initval;
